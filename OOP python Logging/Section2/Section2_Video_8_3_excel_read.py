...
def read_from_excel():
    wb = load_workbook('PythonClass.xlsx')
    ws = wb['Data']
    python_d = {}
    python_d[ws['A1'].value] = ws['B1'].value 
    python_d[ws['A2'].value] = {ws['B2'].value:{ws['C2'].value:ws['D2'].value}}
    python_d['items']['people']['Second'] = ws['D3'].value
    python_d['items']['people']['Third'] = ws['D4'].value
    return python_d
